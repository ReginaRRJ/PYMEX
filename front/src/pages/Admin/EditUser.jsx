...
function EditUser({ user, onClose }) {
  ...
  return (
    <AnimatePresence>
      ...
      >
        <motion.div
          className="bg-white rounded-xl w-[50%] h-[70%]"
        ...
        >
            ...
            <div className="h-full w-[50%] flex flex-col justify-between pl-5 pr-14 pt-3 pb-5">
              ...
              <div className="h-20%">
                ...
                <input
                  type="password"
                  className="w-full h-[3rem] rounded-xl pl-2 bg-slate-200"
                  defaultValue={user.hashContrasena}
                  value={contraseña}
                  readOnly
                />
              </div>
              <div className="h-20% relative">
                <h1
                  className={`${
                    selectedRole === "Sucursal" ? "" : "text-gray-300"
                  }`}
                >
                  Sucursal
                </h1>
                <select
                  type="text"
                  className={`w-full h-[3rem] rounded-xl pl-2 ${
                    selectedRole === "Sucursal"
                      ? "bg-slate-200"
                      : "bg-slate-400 cursor-not-allowed"
                  }`}
                  disabled={selectedRole !== "Sucursal"}
                />
              </div>
            </div>
          </div>
          ...
          <div className="h-[15%] w-full flex justify-center items-center gap-4">
            <button
              id="update-button"
              className="h-[40px] w-[150px] rounded-2xl text-white bg-black hover:bg"
            ...
            <button
              id="delete-button"
              className="h-[40px] w-[150px] rounded-2xl text-white bg-red-500 hover:bg-red-700"
