...
function ReporteSucursal() {
    ...
    const crearReporte = async () => {
        try {
            ...
            const res = await axios.post(`http://localhost:3001/reportes/pedido`, datos);
        ...
        <motion.div className="h-full w-[75%] flex flex-col pt-[6vh] pr-[50px]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}> 
            ...
            <div className="w-full h-[75%] flex flex-col justify-between">
               ...
               <div className="w-full h-[70%]">
                <textarea value={descripcion} onChange={(e) => setDescripcion(e.target.value)} className="w-full h-full rounded-lg px-[1rem] py-[1rem] resize-none bg-slate-200" spellCheck={false} placeholder="Descripción del error"/>
               </div>
               <button className="w-full h-[10%] rounded-2xl text-white bg-blue-600" onClick={crearReporte}>
