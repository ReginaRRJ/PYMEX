...
function Notification({ notification }) {
    const [checked, setChecked] = useState(notification.leida); // fixed from .read

    const handleCheckboxChange = () => {
        if (!checked) {
            setChecked(true);

            // Optionally: send an API call here to mark as read
            // axios.put(`http://localhost:3001/notificaciones/marcar-leida/${notification.idMensaje}`)
        }
    };
